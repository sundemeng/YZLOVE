...
$sexx = ($cook_sex == 1 || empty($cook_sex))?" AND sex=2 ":" AND sex=1 ";
switch ($kind){ 
	case 4:
		$tmpsql = "SELECT $Tfield FROM ".__TBL_MAIN__." WHERE kind=4 $sexx AND flag=1 AND photo_s<>'' ORDER BY zhenghun_jingjia DESC,logintime DESC LIMIT 6";
		$sex1href = "user/?k=4&s=3";
		$sex2href = "user/?k=4&s=4";
	break;
	case 5:
		$tmpsql = "SELECT $Tfield FROM ".__TBL_MAIN__." WHERE kind=5 $sexx AND flag=1 AND photo_s<>'' ORDER BY zhenghun_jingjia DESC,logintime DESC LIMIT 6";
		$sex1href = "user/?k=5&s=3";
		$sex2href = "user/?k=5&s=4";
	break;
	default:
		$tmpsql = "SELECT $Tfield FROM ".__TBL_MAIN__." WHERE kind=2 $sexx AND flag=1 AND photo_s<>'' ORDER BY zhenghun_jingjia DESC,logintime DESC LIMIT 6";
		$sex1href = "user/?k=2&s=3";
		$sex2href = "user/?k=2&s=4";
	break;
	case 1:
		$tmpsql = "SELECT $Tfield FROM ".__TBL_MAIN__." WHERE kind=1 $sexx AND flag=1 AND photo_s<>'' ORDER BY zhenghun_jingjia DESC,logintime DESC LIMIT 6";
		$sex1href = "user/?k=1&s=3";
		$sex2href = "user/?k=1&s=4";
	break;
}
	$rt=$db->query($tmpsql);
	if (!$db->num_rows($rt)){
