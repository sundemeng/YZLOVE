...
if($_REQUEST['submitok']=="delupdate"){
  $coun = count($_POST['list']);
        ...
        {
         //
		  $db->query("delete from  ".__TBL_ASK_BBS__." where id='$list[$i]'");
		}
		echo "<script language=\"javascript\">alert('DELETE成功！');window.location.href='askbbs_search.php";
		echo $kinds;
		echo "'</script>";
		exit();
...
elseif($_REQUEST['submitok']=="sh1"){
	   ...
	   $rst=$db->query("SELECT flag FROM ".__TBL_ASK_BBS__."  where id=".$classid);
       ...
       $db->query("update  ".__TBL_ASK_BBS__."  set flag=".$flag." where id=".$classid);
       header("Location: askbbs_search.php");
	   exit();
}elseif($_REQUEST['submitok']=="jh1"){
       $classid=$_GET['classid'];
	   $rst=$db->query("SELECT ifjh FROM ".__TBL_ASK_BBS__."  where id=".$classid);
	   $rowst = $db->fetch_array($rst);
       if ($rowst[0]==0){
	      $ifjh=1;
	   }else{
	...
	//	   exit;
	   //处
       $db->query("update  ".__TBL_ASK_BBS__."  set ifjh=".$ifjh." where id=".$classid);
       header("Location: askbbs_search.php");
	   exit();
...
else
{
     ...
     if (empty($adminkeyword)){
       $rs=$db->query("SELECT * FROM ".__TBL_ASK_BBS__."  ORDER BY id DESC");
	 }else{
	   $rs=$db->query("SELECT * FROM ".__TBL_ASK_BBS__." WHERE content like '%".$adminkeyword."%' ORDER BY id DESC");
	 }
  ...
  <tr>
    <td width="20%" align="left"><table width="157" height="17" border="0" cellpadding="0" cellspacing="0">
      <tr>
        ...
        <td width="130" style="font-size:10.3pt;padding-top:3px;"><b>求助评论:</b></td>
      </tr>
    </table></td>
...
?>
 <tr bgcolor=#E5E5E5 onMouseOver="this.style.background='#ffffcc'" onMouseOut="this.style.background='#E5E5E5'">
    ...
    <td width="109" align="center"><a href="../clinic/detail<?php echo $rows['fid']; ?>.html" target="_blank" class=u000000><img src="images/zoom.gif" width="13" height="13" hspace="3" border="0" align="absmiddle"><?php echo $rows['fid']; ?></a></td>
    <td width="424"><a href="../clinic/detail<?php echo $rows['fid']; ?>.html" target="_blank" class=u000000><font color="#FF9900">●</font><?php echo $rows['content']; ?></a><?php if ($rows['ifbest']==1){ ?><font color=red>★最佳答案★</font><?php }?></td>
    <td width="29"><a href="javascript:void(0)" onClick="showModalDialog('askbbs_mod.php?classid=<?php echo $rows['id']; ?>', window, 'dialogWidth:800px; dialogHeight:600px;status:0;help:0;scroll:auto;') "><img src="images/fb.gif" hspace="5" vspace="5" border="0" align="absmiddle"></a></td>
