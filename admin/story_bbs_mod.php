...
<?php
  if($_REQUEST['submitok']=="modupdate"){
	 ...
	 echo $title;
	/// exit;
     $db->query("update  ".__TBL_STORY_BBS__."  set content='".$content."' where id=".$classid);
       header("Location: ?classid=".$classid);
	   exit();
  }
  $classid=$_REQUEST['classid'];
  echo $classid;
  $rs=$db->query("SELECT * FROM ".__TBL_STORY_BBS__." WHERE id=".$classid);
...
<table width="750" border="0" align="center" cellpadding="10" cellspacing="0">
  <tr>
    <td valign="top" style="font-size:10.3pt;"><?php echo $rows['content']; ?></td>
  </tr>
</table>
