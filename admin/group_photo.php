...
if($_REQUEST['submitok']=="allupdate"){
        ...
        {
         //删除操作
		  $rt = $db->query("SELECT mainid,path_s,path_b,ifmain FROM ".__TBL_GROUP_PHOTO__." WHERE id=".$list[$i]);
		  if($db->num_rows($rt)){
		   ...
		   $db->query("DELETE  FROM ".__TBL_GROUP_PHOTO__." where id='$list[$i]'");
		}
		echo "<script language=\"javascript\">alert('删除成功！');window.location.href='group_photo.php'</script>";
		exit();
}
     $adminkeyword=$_REQUEST['adminkeyword'];
     if (empty($adminkeyword)){
       $rs=$db->query("SELECT * FROM ".__TBL_GROUP_PHOTO__."  ORDER BY id DESC");
	 }else{
	   $rs=$db->query("SELECT * FROM ".__TBL_GROUP_PHOTO__." WHERE content like '%".$adminkeyword."%' ORDER BY id DESC");
	 }
  ...
  <tr>
    <td width="20%" align="left"><table width="157" height="17" border="0" cellpadding="0" cellspacing="0">
      <tr>
        ...
        <td width="130" style="font-size:10.3pt;padding-top:3px;"><b>群组照片:</b></td>
      </tr>
    </table></td>
...
?>
		...
		<table width="140" border="0" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF" style="border:#dddddd 1px solid;">
          ...
          <tr>
             <td height="18" colspan="2" align="center"><font color="#666666"><?php echo $rows['title']; ?></font></td>
          </tr>
          <tr>
             <td colspan="2" bgcolor="#efefef" bgcolor=#ffffff>来自群组：<b><a href="../group/photo.php?mainid=<?php echo $rows['mainid']; ?>" target="_blank"><img src="images/zoom.gif" hspace="5" border="0" align="absmiddle" ><font color="#FF0000"><u><?php echo $rows['mainid']; ?></u></font></a></b></td>
          <tr>
