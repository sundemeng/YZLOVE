...
<?php
  if($_REQUEST['submitok']=="modupdate"){
	...
	/// exit;
     $db->query("update  ".__TBL_MAIN__."  set aboutus='".$content."' where id=".$classid);
       header("Location: ?classid=".$classid);
	   exit();
  }
  $classid=$_REQUEST['classid'];
  $rs=$db->query("SELECT * FROM ".__TBL_MAIN__." WHERE id=".$classid);
...
<textarea name="content" cols="120" onpropertychange='this.style.posHeight=this.scrollHeight' id=textarea onfocus='textarea.style.posHeight=this.scrollHeight'><?php echo $rows['aboutus']; ?></textarea>
<br>
...
<table width="750" border="0" align="center" cellpadding="10" cellspacing="0">
  <tr>
    <td valign="top" style="font-size:10.3pt;"><?php echo $rows['aboutus']; ?></td>
  </tr>
</table>
