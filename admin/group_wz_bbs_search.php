...
if($_REQUEST['submitok']=="delupdate"){
  $coun = count($_POST['list']);
        ...
        {
         //
		  $db->query("delete from  ".__TBL_GROUP_WZ_BBS__." where id='$list[$i]'");
		}
		echo "<script language=\"javascript\">alert('DELETE成功！');window.location.href='group_wz_bbs_search.php";
		echo $kinds;
		echo "'</script>";
		exit();
...
elseif($_REQUEST['submitok']=="sh1"){
	   ...
	   $rst=$db->query("SELECT flag FROM ".__TBL_GROUP_WZ_BBS__."  where id=".$classid);
       ...
       $db->query("update  ".__TBL_GROUP_WZ_BBS__."  set flag=".$flag." where id=".$classid);
       header("Location: group_wz_bbs_search.php");
	   exit();
	...
	//	   exit;
	   //处
       $db->query("update  ".__TBL_GROUP_WZ_BBS__."  set ifjh=".$ifjh." where id=".$classid);
       header("Location: group_wz_bbs_search.php");
	   exit();
...
else
{
     ...
     if (empty($adminkeyword)){
       $rs=$db->query("SELECT * FROM ".__TBL_GROUP_WZ_BBS__."  ORDER BY id DESC");
	 }else{
	   $rs=$db->query("SELECT * FROM ".__TBL_GROUP_WZ_BBS__." WHERE content like '%".$adminkeyword."%' ORDER BY id DESC");
	 }
  ...
  <tr>
    <td width="20%" align="left"><table width="157" height="17" border="0" cellpadding="0" cellspacing="0">
      <tr>
        ...
        <td width="130" style="font-size:10.3pt;padding-top:3px;"><b>圈子文章评论:</b></td>
      </tr>
    </table></td>
...
?>
  <tr bgcolor=#ffffff onMouseOver="this.style.background='#9EDEFF'" onMouseOut="this.style.background='#ffffff'">
    ...
    <td><a href="####" onClick="showModalDialog('group_wz_bbs_mod.php?classid=<?php echo $rows['id']; ?>', window, 'dialogWidth:800px; dialogHeight:600px;status:0;help:0;scroll:auto;') "><img src="images/fb.gif" hspace="5" vspace="5" border="0"></a><?php echo $rows['content']; ?></td>
