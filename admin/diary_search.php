...
if($_REQUEST['submitok']=="delupdate"){
  ...
  $p=$_REQUEST['p'];
  $db->query("DELETE FROM ".__TBL_DIARY__." where id=".$fid);
   $db->query("DELETE FROM ".__TBL_DIARY_BBS__." where fid=".$fid);
...
elseif($_REQUEST['submitok']=="allupdate"){
        ...
        {
         //
		  $db->query("update ".__TBL_DIARY__." set flag=1 where id='$list[$i]'");
		}
		echo "<script language=\"javascript\">alert('审核成功！');window.location.href='diary_search.php?p=".$p."";
		echo $kinds;
		echo "'</script>";
		exit();
	   ...
	   $p=$_REQUEST['p'];
	   $rst=$db->query("SELECT ifjh FROM ".__TBL_DIARY__."  where id=".$classid);
	...
	//	   exit;
	   //处
       $db->query("update  ".__TBL_DIARY__."  set ifjh=".$ifjh." where id=".$classid);
       header("Location: diary_search.php?p=".$p."");
	   exit();
...
else
{
     ...
     if (empty($adminkeyword)){
       $rs=$db->query("SELECT * FROM ".__TBL_DIARY__."  ORDER BY id DESC");
	 }else{
	   $rs=$db->query("SELECT * FROM ".__TBL_DIARY__." WHERE title like '%".$adminkeyword."%' ORDER BY id DESC");
	 }
  ...
  <tr>
    <td width="20%" align="left"><table width="157" height="17" border="0" cellpadding="0" cellspacing="0">
      <tr>
        ...
        <td width="130" style="font-size:10.3pt;padding-top:3px;"><b>日记管理:</b></td>
      </tr>
    </table></td>
...
?>
 <tr bgcolor=#E5E5E5 onMouseOver="this.style.background='#9EDEFF'" onMouseOut="this.style.background='#E5E5E5'">
    ...
    <td width="326"><a href="../home/diary<?php echo $rows['id']; ?>.html" target="_blank" class=u000000><img src="images/zoom.gif" width="13" height="13" hspace="3" border="0" align="absmiddle"><?php echo $rows['title']; ?></a><font color="#FF0000"><?php echo $rows['xsloveb']; ?></font></td>
    <td width="25"><a href="javascript:(void);" onClick="showModalDialog('diary_mod.php?classid=<?php echo $rows['id']; ?>', window, 'dialogWidth:800px; dialogHeight:600px;status:0;help:0;scroll:auto;') "><img src="images/fb.gif" alt="修改" hspace="2" border="0" align="absmiddle"></a></td>
    ...
    <td width="105" align="center"><?php echo $rows['stime']; ?></td>
	<td width="35" align="center" ><a href="?submitok=delupdate&classid=<?php echo $rows['id']; ?>&p=<?php echo $p;?>" onClick="return confirm('请 慎 重 ！\n\n★确认删除？\n\n请先删除它下面的所有评论。')"><img src="images/delx.gif" alt="删除" width="10" height="10" border="0"></a></td>
 </tr>
 <?php
	}
}else{
