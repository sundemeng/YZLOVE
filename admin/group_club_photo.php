...
if($_REQUEST['submitok']=="allupdate"){
        ...
        {
         //删除操作
		  $rt = $db->query("SELECT mainid,path_s,path_b,ifmain FROM ".__TBL_GROUP_CLUB_PHOTO__." WHERE id=".$list[$i]);
		  if($db->num_rows($rt)){
		   ...
		   $db->query("DELETE  FROM ".__TBL_GROUP_CLUB_PHOTO__." where id='$list[$i]'");
		}
		echo "<script language=\"javascript\">alert('删除成功！');window.location.href='group_club_photo.php'</script>";
		exit();
}
       $rs=$db->query("SELECT * FROM ".__TBL_GROUP_CLUB_PHOTO__."  ORDER BY id DESC");
  ...
  <tr>
    <td width="20%" align="left"><table width="157" height="17" border="0" cellpadding="0" cellspacing="0">
      <tr>
        ...
        <td width="130" style="font-size:10.3pt;padding-top:3px;"><b>活动照片:</b></td>
      </tr>
    </table></td>
...
?>
		...
		<table width="140" border="0" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF" style="border:#dddddd 1px solid;">
          ...
          <tr>
        <td height="18" colspan="2" align="center"  bgcolor=#ffffff><font color="#666666"><?php echo $rows['title']; ?></font></td>
      </tr>
          <tr>
        <td height="18" colspan="2" align="center"><font color="#666666"><?php echo $rows['addtime']; ?></font></td>
      </tr>
          <tr>
             <td colspan="2" bgcolor="#efefef" bgcolor=#ffffff>来自活动：<b><a href="../group/partyphoto.php?fid=<?php echo $rows['mainid']; ?>" target="_blank"><img src="images/zoom.gif" hspace="5" border="0" align="absmiddle" ><font color="#FF0000"><u><?php echo $rows['mainid']; ?></u></font></a></b></td>
          <tr>
             <td width="60" height="5" align="right" bgcolor="#efefef"  style="color:#ff0000" title="主照片"></td>
             <td width="90" align="right" bgcolor="#efefef"><input type=checkbox name=list[] value="<?php echo $rows['id']; ?>" id="chk<?php echo $rows['id']; ?>" style="border:0px"><label for="chk<?php echo $rows['id']; ?>"><font color="#999999">选择</font></label></td>
          </tr>
        </table>
		</td>
<?php	
